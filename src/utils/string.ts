...
/**
 * Calculates the uptime in a human-readable format based on the provided start time.
 * The function returns a string with the time difference as days, hours, minutes, and seconds, excluding zero units.
 *
 * @param startTime - The starting time (in ISO string format) to calculate uptime from.
 * @returns A string representing the uptime in a human-readable format.
 *
 * @example
 * calculateUptime('2025-03-27T15:54:15.710Z') // returns "1 day, 5 hours, 12 minutes, 20 seconds"
 */
export const calculateUptime = (startTime: string): string => {
  ...
  if (hours % 24 > 0)
    timeParts.push(`${hours % 24} ${hours % 24 === 1 ? 'hour' : 'hours'}`)
  if (minutes % 60 > 0)
    timeParts.push(
      `${minutes % 60} ${minutes % 60 === 1 ? 'minute' : 'minutes'}`
    )
  if (seconds % 60 > 0 || timeParts.length === 0)
    timeParts.push(
